...
func getProjectInstances() ([]Instance, error, string) {
	...
	return linuxInstances, nil, project
}
...
func main() {
	...
	instances, err, project := getProjectInstances()
	if err != nil {
	...
	// CSV output
	filename := fmt.Sprintf("disk_usage_report_%s_projeto_%s.csv",
		time.Now().Format("20060102_150405"), project)
