...
func getLinuxDiskUsage(instance Instance) []Disk {
	...
	if !testSSHConnection(instance) {
		fmt.Printf("Skipping %s due to SSH connection failure\n", instance.Name)
		return nil
	}
...
func testSSHConnection(instance Instance) bool {
	cmd := fmt.Sprintf(`gcloud compute ssh %s --zone=%s --tunnel-through-iap --command='exit' --quiet \
        --ssh-flag="-o BatchMode=yes -o ConnectTimeout=10 -o PasswordAuthentication=no"`,
		instance.Name, instance.Zone)
	...
	if err != nil {
		if strings.Contains(strings.ToLower(err.Error()), "permission denied") ||
			...
			fmt.Printf("SSH auth failed for %s (password required). Skipping.\n", instance.Name)
			return false
		}
